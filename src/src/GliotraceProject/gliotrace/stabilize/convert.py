...
def stack_conversion(inpath, outpath, metadata_path):
    ...
    new_exps = np.unique([int(re.search(r"exp_(\d+)",f.name).group(1)) for f in to_process])
    old_exps = np.unique([int(re.search(r"exp_(\d+)",f).group(1)) for f in old_names])

    # Create folders if missing
    metadata = pd.read_csv(new_path)
    old_sets = np.unique([metadata.loc[metadata["experiment_id"] == exp, "set"].values[0] for exp in old_exps])
    new_sets = np.unique([s for exp in new_exps if (s := metadata.loc[metadata["experiment_id"] == exp,"set"]) not in old_sets])
    ...
    for f in to_process:
        try:
            ...
            with h5py.File(f,'r') as f_h5:
                tstack = np.clip(np.array(f_h5['stack']['Tstack']), 0, 255).astype(np.uint8)
                vstack = np.clip(np.array(f_h5['stack']['Vstack']), 0, 255).astype(np.uint8)

        bstack = np.zeros_like(tstack)

        exp = int(re.search(r"exp_(\d+)",f.name).group(1))
        set = metadata.loc[metadata["experiment_id"] == exp, "set"]
        delta = metadata.loc[metadata["experiment_id"] == exp, "delta_t"]
...
def read_inhouse_metadata(metadata_path):
    ...
    cols_select = [c for c in data.columns if any(c.lower() == p.lower() or c.lower().startswith(f"{p.lower()}_") for p in cols_)]
    filtered_data = data[cols_select]

    # Replace experiment column name
    exp_idx = next(i for i,c in enumerate(filtered_data.columns) if c.startswith('exp'))
    filtered_data = filtered_data.rename(columns={filtered_data.columns[exp_idx] : "experiment_id"})
    filtered_data["experiment_id"] = filtered_data["experiment_id"].str.extract(r"(\d+)").astype(int)
