...
def select_stabilize(
    ...
    for folder, tif_paths in groups.items():
        ...
        if mode == "full":
            ...
            if sample.ndim != 3:
                raise ValueError(
                    f"Expected RGB image, got {im.shape} at {tif_paths[0]}"
                )
            ...
            if sample.ndim != 3:
                raise ValueError(
                    f"Expected RGB image, got {sample.shape} at {tif_paths[mid_idx]}"
                )
        ...
        for k, roi in enumerate(rois, start=1):
            ...
            for t_idx, tif_path in enumerate(tif_paths, start=1):
                ...
                if im.ndim != 3:
                    raise ValueError(
                        f"Expected Height, Width, Color, got {im.shape} at {tif_path}"
                    )
