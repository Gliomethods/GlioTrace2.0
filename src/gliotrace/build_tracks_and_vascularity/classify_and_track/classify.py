...
def classify_tumor_cells(
    ...
    """
    # Load mean images (assumed shape [H, W, 3])
    SCRIPT_DIR = Path(__file__).resolve().parent

    # Build full paths
    mean_image_6class_path = SCRIPT_DIR / "mean_image_6class.mat"
    mean_image_tme_path = SCRIPT_DIR / "mean_image_tme.mat"

    # Load
    mean_image_6class = loadmat(mean_image_6class_path)["mean_image"]
    ...
    # One list of DataFrames per frame
    ...
    with torch.no_grad():
        ...
        for i, (mat, mat_vasc) in enumerate(zip(feat, vasc), start=1):
            ...
            frame_rows_meta = []  # rows for metadata DF for this frame
            ...
            for j in range(n_cells):
                ...
                # Normalize and mean-subtract
                im_norm = im / 255.0
                im_predict = im_norm - mean_image_6class
                im_predict_tme = im_norm - mean_image_tme

                # ---- PyTorch tensor: [B, C, H, W] ----
                # Morphology input
                im_morph_tensor = (
                    torch.from_numpy(im_predict)
